...
for tc in range(1, T+1):
    ...
    while m:
        ...
        for i in range(len(visited)):
            dic[visited[i]] = visited.count(visited[i]) # 방문하는 점 개수 파악

        for i in range(len(arr)):
            if arr[i][2]:                               # 이 값이 있다면, 탐색하는 점이라는 뜻
                x, y = arr[i][0], arr[i][1]
                nx = x + direction[arr[i][3]][1]
                ny = y + direction[arr[i][3]][0]
                if 1 <= nx < n-1 and 1 <= ny < n-1:     # 범위 내에 있는 값이라면
                    if dic[(nx, ny)] == 1:              # 값이 하나만 있다면
                        square[nx][ny] = arr[i][2]
                        arr[i][0], arr[i][1] = nx, ny
                    else:                               # 값이 여러개가 있다면
                        square[nx][ny] += arr[i][2]     # 값을 더해준다
                        arr[i][0], arr[i][1] = nx, ny
                        for j in range(len(arr)):
                            if arr[j][0] == nx and arr[j][1] == ny and i != j and arr[j][2] != 0:
                                if arr[i][2] > arr[j][2]:
                                    arr[j][2] = 0
                                    king = i
                                else:
