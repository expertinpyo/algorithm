...
# 문제점 : 섬 전체에 대한 거리를 작성하지 못한다.
# 무슨말이냐면 1과 2가 1칸 떨어진 부분이 있는데, 다른 칸에서 그 이상 떨어져 있으면
# 그 부분을 무시하게 된다.
# 즉 count가 된다는 말이다.
                    ...
                    if island_num == -1:    # 처음 마주치는 섬일 때
                        ...
                        if island_list[k] == island_num:    # 같은 번호의 섬이라면, idx만 갱신
                            ...
                            if abs(idx-k) - 1 > 1:   # 거리가 1보다 큰 경우에는
                                ...
                                if b not in edges:
                                    edges.append(b)
                            # elif abs(idx-k) -1 == 1 and [island_num, island_list[k]] not in prohibited and [island_list[k], island_num] not in prohibited:
                            #     prohibited.append([island_num, island_list[k]])
                            #     prohibited.append([island_list[k], island_num])
                            idx = k
                            island_num = island_list[k]
